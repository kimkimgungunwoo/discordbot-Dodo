...
CATEGORIES: dict[str, tuple[str, str]] = {
    ...
    "party": (
        "👥 파티",
        "`!파티 생성` — 입력창에서 제목·날짜·오전오후·시·분 지정 → 초대할 사람·역할(@everyone 포함) 선택\n"
        "`!파티 목록` — 파티장·참가자 수 카드로 확인\n"
        "`!파티 삭제` — 드롭다운으로 파티 선택해 삭제 (시간 가까운 순)\n"
        "`!파티 멤버` — 드롭다운으로 파티 선택해 참여자 확인",
    ),
    "ai": (
        "🤖 AI 챗봇",
    ...
    "overwatch": (
        "🎮 오버워치",
        "`!오버워치 프로필` — 배틀태그 검색 후 프로필 확인 / 즐겨찾기 목록에서 재조회\n"
        "`!오버워치 즐겨찾기` — 즐겨찾기 계정 목록 확인 / 추가\n"
        "`!오버워치 분석` — 프로필 + AI 코멘트 (역할별 성적·모스트 영웅 종합 분석)\n"
        "`!오버워치 영웅분석` — 플레이시간 상위 10개 영웅 + AI 코멘트",
    ),
    "analytics": (
        "📊 서버 통계",
        "`!통계 채팅통계` — 전체 메시지 수·채팅 많은 유저 순위\n"
        "`!통계 통화통계` — 전체 통화시간·통화 많은 유저 순위\n"
        "`!통계 유저통계` — 드롭다운으로 유저를 선택해 개인 통계 (통화메이트 순위 포함)\n"
        "`!통계 서버전체통계` — 채팅+통화 합산 대시보드 (베스트 커플·인기 게임 포함)\n"
        "> 채팅은 과거 메시지도 자동 집계되지만, 통화·게임은 기능이 켜진 이후부터만 기록됩니다.",
    ),
    "etc": (
        "💬 기타",
