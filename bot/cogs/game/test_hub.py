...
class LobbyTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_buttons_and_player_permissions(self):
        ...
        self.assertEqual(link_button.url, f"https://dodobirdactivity.test/volleyball?room={self.room_id}")
    ...
    async def test_stale_room_click_closes_the_orphaned_message(self):
        # 봇 재시작 등으로 self.rooms에서 이미 사라진 방의 버튼을 눌렀을 때 — 응답만 주고 끝내면
        # 메시지엔 죽은 버튼이 영원히 남는다. 그 메시지 자체를 정리(버튼 제거)해야 한다.
        interaction = self.interaction(1)
        interaction.message = AsyncMock(embeds=[SimpleNamespace(title="도도새오목")])
        await self.cog.action("no-such-room", "spectate", interaction)
        interaction.message.edit.assert_awaited_once()
        _, kwargs = interaction.message.edit.call_args
        self.assertIsNone(kwargs["view"])
        self.assertEqual(kwargs["embed"].title, "도도새오목")
        self.assertEqual(kwargs["embed"].description, "종료된 방입니다.")
        interaction.followup.send.assert_awaited_once_with("종료된 방입니다.", ephemeral=True)

    async def test_waiting_timeout_removes_room(self):
        task = self.cog.room_tasks[self.room_id]
        task.cancel()
        await asyncio.gather(task, return_exceptions=True)
        with patch("bot.cogs.game.asyncio.sleep", new=AsyncMock()):
