    ...
    async def cpu(self, interaction, button):
        async with self.selection_lock:
            if self.selected:
                ...
                return
            self.clear_items()
            for difficulty, label in GAMES[self.game].difficulty_labels.items():
            ...
            await interaction.response.edit_message(
                embed=discord.Embed(title=f"도도새{GAMES[self.game].label} · 봇전", description="난이도를 선택해주세요."), view=self)
    ...
    async def choose(self, interaction, mode, difficulty="normal"):
        ...
        async with self.selection_lock:
            if self.selected:
                ...
                return
            room = await self.cog.create_room(self.ctx, mode=mode, difficulty=difficulty, game=self.game)
            if room is None:
        ...
        if mode == "CPU":
            await interaction.followup.send(f"게임을 시작했습니다. 위 방 메시지의 [{GAMES[self.game].label} 하러 가기] 버튼으로 들어오세요.", ephemeral=True)
        else:
...
class Game(commands.Cog):
    ...
    async def select_mode(self, ctx: commands.Context, game="volleyball"):
        if not ctx.guild:
            ...
            return
        view = ModeView(self, ctx, game)
        self.selection_views.add(view)
        try:
            view.message = await ctx.send(
                embed=discord.Embed(title=f"도도새{GAMES[game].label}", description="봇전 또는 대결을 선택해주세요."), view=view)
        except Exception:
            view.stop()
            raise
    ...
    async def _mark_message_closed(self, message: discord.Message | None):
        if message is None:
            return
        title = message.embeds[0].title if message.embeds else "도도새"
        try:
            await message.edit(embed=discord.Embed(title=title, description="종료된 방입니다."), view=None)
        except discord.HTTPException:
            log.warning("Could not close stale Dodo lobby message")
    ...
    async def action(self, room_id, action, interaction, *, deferred=False):
        ...
        async with self._lock_for(self._guild_of(room_id)):
            ...
            if action == "spectate":
                ...
                note = f"위 방 메시지의 [{game_name(room)} 하러 가기] 버튼으로 들어오세요." if room["status"] == "PLAYING" else "경기가 시작되면 방 메시지의 게임 링크로 들어오세요."
            ...
            try:
                ...
                return
            await interaction.followup.send(f"게임을 시작했습니다. 위 방 메시지의 [{game_name(room)} 하러 가기] 버튼으로 들어오세요.", ephemeral=True)
