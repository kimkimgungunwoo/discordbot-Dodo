...
- `!게임 오목` → 봇전(쉬움 / 중간 / 어려움 / 극한 / 초월) 또는 대결 → **오목 하러 가기** 링크.
- `/omok`은 로그인 없는 CPU 연습, `/omok?room=...`은 Discord 인증을 거치는 온라인 방이다.
- 15×15, 흑 선공. 흑은 정확한 5목, 백은 5목 이상이면 승리한다. 흑의 33·44·장목은 착수할 수 없고 보드에 ×로 표시한다. AI와 45초 자동 착수에도 같은 금수를 적용한다. 다음 플레이어의 합법 수가 없으면 무승부다.
- 서버가 경기마다 동전 던지기로 흑백을 배정한다. 참가자가 연결되면 4초간 추첨 연출(마지막 2초는 결과 공개) 후 시작한다. 한 수는 45초 안에 두지 않으면 서버가 대신 무작위로 둔다.
- 서버가 차례·착수·승패를 판정한다. `MOVE {matchId, revision, at}`에 대해 전체 보드 상태를 전달하며 관전·재접속 시 현재 상태를 받는다. 배구의 실시간 틱 프로토콜과 분리되어 있다.
- 방 생성, OAuth, 길드 멤버 확인, 결과 콜백, PVP 양측 동의 재경기는 배구와 같은 로비를 사용한다.
- OAuth는 게임별로 각자의 Redirect URI(`/volleyball`, `/omok`)를 쓴다 — Discord 앱 설정에 둘 다 등록해야 한다.
- `shared/omok.ts`: 연습 및 서버 공통 규칙/AI. 쉬움은 후보 선택에 오차, 중간·어려움은 고정 깊이 탐색, 극한은 최대 0.5초 반복 심화 탐색을 사용한다. 초월은 최대 6초, 최대 20수 깊이의 탐색과 24수 연속 4 공격 증명, 국면 캐시, 전술 연장 탐색을 사용한다. 시간 안에 끝난 탐색 결과만 채택한다. 서버의 극한·초월과 브라우저 연습 AI는 별도 워커에서 계산한다. 완전 탐색이나 무패를 보장하는 엔진은 아니다.
- `shared/renju.ts`: [RIF 9.2–9.3](https://www.renju.net/rifrules/) 기준 금수 판정. 정확한 5목을 우선하고, 열린 4로 합법적으로 확장할 수 없는 가짜 3은 33에서 제외한다. 동전 추첨 방식은 유지하며 대회 오프닝·패스 규칙은 적용하지 않는다.
- `client/src/omok/`: 배구 팔레트와 도도새 초상화를 사용하는 화면. `server/src/omok-room.ts`: 서버 판정 및 CPU 진행.
- 게임 상태는 메모리에만 보관하며 서버 재시작 시 소멸한다. 5분간 착수가 없으면 중단된다.

```sh
docker compose up -d activity-client activity-server bot
docker compose exec -T activity-server npm test
docker compose exec -T bot python -m unittest bot.cogs.game.test_hub bot.cogs.game.test_omok
```

`ACTIVITY_PUBLIC_URL=http://localhost:5173`이면 게임 링크는 이 PC에서만 사용할 수 있다. 다른 기기의 참가자를 받으려면 공개 도메인과 해당 도메인의 `/volleyball` OAuth 복귀 주소가 필요하다.
...
`http://localhost:5173`(도메인 루트)은 기존 CPU 연습 모드다. 실제 배구 진입점은 `/volleyball`이고(`?room=<roomId>` 쿼리로 채팅 링크에서 들어옴), 루트는 나중에 다른 게임이 쓸 수 있게 비워둔다.

**Discord Developer Portal 설정**: Activities 기능/URL Mapping은 필요 없다. 대신 **OAuth2 → Redirects**에 아래 두 개를 등록한다(끝에 `/volleyball`까지 정확히 일치해야 함):
```
https://<운영 도메인>/volleyball
http://localhost:5173/volleyball
```
...
모든 난이도는 같은 플레이어 물리를 사용한다. AI 판단은 상태와 게임 틱만 사용하며 관전/재접속/화면 예측에서도 동일하게 재현된다. `cd games/client && node tests/ai-benchmark.mjs`로 기존 보통 AI 상대 좌우·서브 교대 비교를 실행할 수 있다. 이는 제한된 자동 비교이며 사람 상대 체감 난이도는 플레이 테스트로 조정한다.
...
- 경기는 7점 선취(`WIN_SCORE`, client `volleyball/constants.ts`/server `volleyball-room.ts`/bot `game/volleyball.py` 세 군데가 같은 값이어야 함)다. PVP 결과는 양쪽의 종료 tick/점수가 일치해야 확정한다. CPU 결과는 host가 보고한다. 콜백 실패 시 5초 간격으로 재시도한다. 종료 방은 재대결을 위해 잠시 유지하며 같은 방 ID를 새 seed로 재사용한다.
...
docker compose exec -T bot python -m unittest bot.cogs.game.test_hub   # 또는 discordvenv/bin/python -m unittest ...
```
...
- 방장 한 명당 방 하나만 유지한다. 새 방 생성 시 게임 종류·채널·서버·진행 상태에 관계없이 기존 방의 웹 세션과 Discord 대기방을 먼저 닫는다. 기존 세션 종료를 확인할 수 없으면 중복 방을 만들지 않고 재시도를 안내한다.
...
- 두 게임의 관전자 목록은 Discord 프로필 사진·서버 닉네임·인원수를 표시하고 입장/퇴장을 실시간 반영한다. 처음에는 `소리 켜기 ♪`를 누르거나 화면을 터치해야 브라우저가 오디오를 허용한다. 이후 실시간 경기 효과음을 재생하고 중도 입장·재접속의 과거 효과음은 생략한다.
...
- `games/client/Caddyfile`: 실제 도메인 기준 자동 HTTPS. `/api/*`, `/ws`만 activity-server로 프록시하고 `/internal/*`는 아예 라우팅하지 않아 외부에서 접근 불가. `/terms`, `/privacy`(App Verification용 공개 페이지, `games/client/public/`), `/volleyball`(배구 진입점)도 여기서 정적 서빙. 도메인 루트는 다른 게임을 위해 비워둠 — 새 게임 추가 시 같은 패턴으로 `handle /<새경로>` 블록만 추가하면 됨.
